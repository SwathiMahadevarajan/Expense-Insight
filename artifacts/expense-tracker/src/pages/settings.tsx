      ...
      <CardContent className="space-y-4">
        ...
        <div className="border rounded-xl p-4 space-y-3">
          ...
          {notifPermission === "denied" && (
            <p className="text-xs text-muted-foreground">Notifications are blocked. Go to your browser or device settings to allow them for this site.</p>
          )}
          {notifPermission === "granted" && (
            <p className="text-xs text-green-700 bg-green-50 border border-green-200 rounded-lg px-3 py-2">
              ✅ Notifications are active. Reminders fire while SmartTrack is open in the browser.
            </p>
          )}
        </div>

        {/* iOS notification reality note */}
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 space-y-1.5">
          <p className="text-sm font-semibold text-amber-800">📱 iOS notification support</p>
          <p className="text-xs text-amber-700">
            iOS 16.4+ supports notifications for installed PWAs — but only when the app is added to your Home Screen via Safari's Share menu.
          </p>
          <ul className="text-xs text-amber-700 space-y-0.5 list-disc list-inside">
            <li>Open SmartTrack in <strong>Safari</strong> on your iPhone</li>
          ...
          <p className="text-xs text-amber-600 mt-1">⚠️ SmartTrack's reminders fire while the app is open. True background push requires a push server, which is not set up in this local-first version.</p>
        </div>
      </CardContent>
    </Card>
  );
...
function NotificationsSection() {
  ...
  const handleToggleActive = async (notif: NotificationSetting) => {
    ...
    toast({ title: notif.isActive ? "Reminder paused" : "Reminder enabled" });
  };

  const handleEdit = (notif: NotificationSetting) => {
          ...
          <div className="space-y-3">
            {!data?.notifications.length ? (
              <div className="text-center py-8 space-y-2">
                ...
                <p className="text-xs text-muted-foreground">Add a daily reminder to log your expenses.</p>
              </div>
            ) : data?.notifications.map(notif => (
              <div key={notif.id} className={`flex items-start justify-between p-4 border rounded-xl transition-colors ${notif.isActive ? "" : "opacity-60 bg-muted/20"}`}>
                ...
                <div className="flex items-center gap-1 ml-3 flex-shrink-0">
                  <Button
                    ...
                    title={notif.isActive ? "Pause reminder" : "Enable reminder"}
                  ...
                  <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => handleEdit(notif)}>
                    <Edit2 className="w-3.5 h-3.5" />
                  </Button>
...
function NotificationDialog({
  ...
  notificationToEdit: NotificationSetting | null;
}) {
  const { toast } = useToast();
  ...
  React.useEffect(() => {
    if (open) {
      ...
      setFrequency(notificationToEdit?.frequency ?? "daily");
      setTime(notificationToEdit?.time ?? "09:00");
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (notificationToEdit) {
        await updateNotification(notificationToEdit.id, { title, message, frequency, time });
  ...
  const freqHint = frequency === "weekly" ? "Fires every Monday" : frequency === "monthly" ? "Fires on the 1st of each month" : "Fires every day";

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{notificationToEdit ? "Edit Reminder" : "New Reminder"}</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4 pt-2">
          <div className="space-y-2">
            <Label>Title</Label>
          ...
          <p className="text-xs text-muted-foreground">{freqHint} at {time}. The app must be open in the browser for reminders to fire.</p>
          <div className="flex justify-end gap-2">
            <Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button>
