...
import {
  GetInsightsSummaryQueryParams,
  GetSpendingByCategoryQueryParams,
  GetDailySpendingQueryParams,
} from "@workspace/api-zod";
import { eq, and, gte, lte, sql, sum } from "drizzle-orm";

const router: IRouter = Router();

function getDateRange(period: string, startDate?: string, endDate?: string): { start: Date; end: Date; label: string } {
  const now = new Date();
  let start: Date;
...
router.get("/insights/summary", async (req, res) => {
  try {
    ...
    const dateConditions = and(gte(transactionsTable.date, start), lte(transactionsTable.date, end));
    ...
    const topCategory = topCategoryResult[0]?.categoryName ?? null;
    const topCategoryAmount = topCategoryResult[0]?.total ? Number(topCategoryResult[0].total) : null;
    ...
    const accountBalances = await Promise.all(
      accounts.map(async (account) => {
        ...
        const netChange = Number(result[0]?.netChange ?? 0);
        return {
          accountId: account.id,
          accountName: account.name,
          balance: account.openingBalance + netChange,
        };
      })
    );
